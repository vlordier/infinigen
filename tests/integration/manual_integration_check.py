...
def test_generation(dir):
    ...
    logger.info(f'{num_lines}/{num_scenes} succeeded scenes')
...
def test_logs(dir):
    ...
    logger.info('Time Logs by Step')
    logger.info(tabulate(step_stats, headers='keys', tablefmt='fancy_grid'))
    ...
    logger.info(tabulate(asset_stats.sort_values('% of scene time', ascending=False), headers='keys', tablefmt='fancy_grid'))
    ...
    logger.info(tabulate(assset_mem_stats.applymap(lambda x: sizeof_fmt(x)), headers='keys', tablefmt='fancy_grid'))
    ...
    logger.info(tabulate(obj_created_stats, headers='keys', tablefmt='fancy_grid'))
    ...
    logger.info(tabulate(instance_created_stats, headers='keys', tablefmt='fancy_grid'))

    poly_df = pd.DataFrame.from_dict(poly_data, orient="index")
    poly_stats = make_stats(poly_df)
    logger.info('\nPolycount Statistics')
    logger.info(tabulate(poly_stats, headers='keys', tablefmt='fancy_grid'))
...
def test_step_memory(dir, days):
    ...
    logger.info(f'Running + {sacct_command}')
    ...
    logger.info(tabulate(step_mem_stats, headers='keys', tablefmt='fancy_grid'))
    ...
    for scene in os.listdir(dir):
        for filepath in Path(os.path.join(dir, scene)).rglob("Image*.png"):
            ...
            if meanPercent < 5:
                ...
                logger.info(f'{scene} is dark')
            else:
                all_data[scene]["Dark"] = False

    logger.info(f'{numDark}/{num_lines} images are dark')
...
def test_noise(dir):
    ...
    logger.info(tabulate(noise_df.sort_values('Noise Estimate', ascending=False), headers='keys', tablefmt='fancy_grid'))
...
def test_gt(dir):
    ...
    logger.info('Comparison checking not fully working, proceed with caution')
    similarity_df.columns = ["Similarity Score (%)"]
    logger.info(tabulate(similarity_df, headers='keys', tablefmt='fancy_grid'))
    ...
    logger.info('Percent of untagged pixels: ' + str(100 - tag_seg_stats['Percent of Pixels'].sum()))
    logger.info(tabulate(tag_seg_stats.sort_values('Percent of Pixels', ascending=False), headers='keys', tablefmt='fancy_grid'))
    ...
    logger.info('Percent of untagged pixels: ' + str(100 - obj_seg_stats['Percent of Pixels'].sum()))
    logger.info(tabulate(obj_seg_stats.sort_values('Percent of Pixels', ascending=False), headers='keys', tablefmt='fancy_grid'))
