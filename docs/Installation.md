...
Then run the container without GPU passthrough or OpenGL ground truth (NVIDIA GPU passthrough and OpenGL-based ground truth annotations are not available in Docker Desktop on Apple Silicon):
```bash
make docker-run-no-gpu-opengl
```

Note: CUDA and OpenGL ground truth annotations are not supported on ARM64. Use `make docker-build` (not `docker-build-cuda`) for ARM64 builds.
